...
export class MongoDbService implements IPostDao {
  ...
  async findAll(): Promise<Array<Posts>> {
    try {
      const results = await this._postModel.find();
      return results;
    } catch (error) {
      if (error instanceof mongo.MongoError) mongoExceptionHandler(error);
      else throw error;
  ...
  async findByPhone(phone: string): Promise<Posts> {
    try {
      const post = await this._postModel.findOne({ phone: phone });
      return post;
    } catch (error) {
      if (error instanceof mongo.MongoError) mongoExceptionHandler(error);
      else throw error;
  ...
  async update(
    ...
    try {
      const post = await this._postModel.findByIdAndUpdate(
        id,
        updatePostDto,
      );
      return post;
